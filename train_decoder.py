...
if 1:
    ...
    train         = EasyDict(run_func_name='training.training_loop_decoder.training_loop')         # Options for training loop.
    G             = EasyDict(func_name='training.networks_stylegan.G_style')               # Options for generator network.
    D             = EasyDict(func_name='training.networks_stylegan.D_basic')               # Options for discriminator network.
    NE            = EasyDict(func_name='training.networks_stylegan.G_mapping_NE')          # Options for invertible encoder network.
    ...
    #desc += '-car';      dataset = EasyDict(tfrecord_dir='lsun-car-128x96');     train.mirror_augment = False
...
def main():
    ...
    kwargs.update(NE_args=NE, G_args=G, D_args=D, G_opt_args=G_opt, D_opt_args=D_opt, G_loss_args=G_loss, D_loss_args=D_loss)
