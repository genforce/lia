...
def training_loop(
    submit_config,
    NE_args                 = {},       # Options for flow decoder network.
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G, D, Gs, NE = misc.load_pkl(network_pkl)
        else:
            print('Constructing networks...')
            NE = tflib.Network('NE', latent_size=512, depth=4, **NE_args)
            G = tflib.Network('G', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, latent_size=512, depth=4, **G_args)
            D = tflib.Network('D', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, **D_args)
            Gs = G.clone('Gs')
    G.print_layers(); D.print_layers(); NE.print_layers()
    ...
    # Write final results.
    misc.save_pkl((G, D, Gs, NE), os.path.join(submit_config.run_dir, 'network-final.pkl'))
    summary_log.close()
